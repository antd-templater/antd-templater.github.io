import __vue_component__$1 from '../S-Ellipsis/index.mjs';
...
var script = {
  ...
  methods: {
    ...
    cleanNodeConfig(force) {
      ...
      // 清理中 - loadKeys
      this.loadKeys.splice(0, this.loadKeys.length, ...this.loadKeys.filter(value => this.flatTreeNodes.some(every => every.value === value && Util.isNotEmptyArray(every.children))));

      // 清理中 - loadedKeys
      this.loadedKeys.splice(0, this.loadedKeys.length, ...this.loadedKeys.filter(value => this.flatTreeNodes.some(every => every.value === value && Util.isNotEmptyArray(every.children))));

      // 清理中 - expanded
      this.expandedKeys.splice(0, this.expandedKeys.length, ...this.expandedKeys.filter(value => this.flatTreeNodes.some(every => every.value === value && Util.isNotEmptyArray(every.children))));
    ...
    reloadTreeNodes(nodes, parent) {
      ...
      if (Util.isNotEmptyObject(parent) && !this.flatTreeNodes.some(every => every.value === parent.value)) {
        return Promise.resolve();
      }
      if (!Util.isNotEmptyObject(parent)) {
        if (nodes !== this.treeNodes) {
    ...
    compileTreeNodes(nodes, parent) {
      ...
      for (const item of nodes) {
        ...
        // selectable
        item.selectable = Util.isFunction(this.allowSelectedLevel) ? !!this.allowSelectedLevel(item) : item.level >= this.allowSelectedLevel;

        // checkable
        item.checkable = Util.isFunction(this.allowCheckedLevel) ? !!this.allowCheckedLevel(item) : item.level >= this.allowCheckedLevel;
    ...
    expandTreeNodes(expandedKeys) {
      if (Util.isNotEmptyArray(expandedKeys)) {
        for (const expandedKey of expandedKeys) {
          ...
          if (selectedNode && (Util.isNotEmptyArray(selectedNode.children) || isAsyncNode && !isLoadedNode)) {
            ...
            if (this.allowAutoExpanded) {
              ...
              while (onlyOneChild && Util.isNotEmptyObject(firstChildNode) && Util.isNotEmptyArray(firstChildNode.children) && !expandedKeys.includes(firstChildNode.value)) {
    ...
    doTreeAllExpand(expandedKeys) {
      this.expandedKeys.splice(0, this.expandedKeys.length, ...this.flatTreeNodes.filter(every => Util.isNotEmptyArray(every.children)).map(every => every.value));
    },
    /**
      ...
      if (Util.isNotEmptyArray(addExpandedNodes)) {
        if (!this.allowMultiExpanded) {
          ...
          if (Util.isNotEmptyArray(childs) || isAsyncNode && !isLoadedNode) {
            ...
            expandKeys.push(...childs.filter(child => addExpandedKeys.includes(child.value)).filter(child => Util.isNotEmptyArray(child.children)).map(node => node.value));
          }
        ...
        if (this.allowMultiExpanded) {
          ...
          for (const value of addExpandedKeys) {
            ...
            if (Util.isNotEmptyArray(childs) || isAsyncNode && !isLoadedNode) {
              ...
              expandKeys.push(...childs.filter(child => addExpandedKeys.includes(child.value)).filter(child => !expandKeys.includes(child.value)).filter(child => Util.isNotEmptyArray(child.children)).map(node => node.value));
            }
          }
    ...
    doTreeSelect(selectedKeys) {
      ...
      if (!Util.isNotEmptyObject(newFirstSelectedNode)) {
        !allowUnSelected && Util.isNotEmptyObject(oldFirstSelectedNode) ? this.selectedNodes.splice(0, this.selectedNodes.length, oldFirstSelectedNode) : this.selectedNodes.splice(0, this.selectedNodes.length);
      }
            ...
            if (Util.isNotEmptyArray(nodes) && this.flatTreeNodes.some(findNode)) {
              if (this.allowAutoExpanded) {
                Promise.resolve().then(() => {
                  this.doTreeExpand([loadNode.value, ...this.expandedKeys, ...nodes.filter(node => nodes.length === 1 && node.isLeaf !== true && Util.isFunction(this.loadData)).map(node => node.value)]);
                });
              }
