...
var T = require('ant-design-vue/es/table/Table');
...
var STable = {
  ...
  methods: {
    ...
    refresh(bool = false, delay = 0) {
      if (bool === true) {
        if (['auto', 'imitate', true].includes(this.showPagination)) {
          ...
          delay > 0 ? setTimeout(() => this.loadData({
    ...
    loadData(pagination = {}, filters = {}, sorter = {}) {
      ...
      const pageNo = this.localPagination && this.localPagination.current || this.pagination.current;
      const pageSize = this.localPagination && this.localPagination.pageSize || this.pagination.pageSize;
      ...
      Promise.resolve(this.data(parameter)).then(response => {
        Promise.resolve(this.handleData(response, base["default"])).then(result => {
          Object.assign(this.pagination, {
          ...
          if (this.pagination.pageSize) {
            ...
            if (this.pagination.current > totalValue && this.pagination.current > 1) {
    ...
    updateExpand(expandedRowKeys) {
      if (base["default"].isArray(this.expandedRowKeys) && base["default"].isArray(expandedRowKeys)) {
    ...
    initColumnField(columns) {
      for (const column of columns) {
        if (!base["default"].isArray(column.children) || column.children.length === 0) {
      ...
      for (const column of columns) {
        if (!base["default"].isArray(column.children) || column.children.length === 0) {
          if (this.optionColumns.some(option => option.value === column.dataIndex)) {
            if (this.selectColumns.length === 0 || this.selectColumns.some(select => select.value === column.dataIndex && select.filter !== false)) {
              columnArr.push({
                ...column
              });
            }
  ...
  render() {
    ...
    // 处理 组件参数
    Object.keys(T__default["default"].props).forEach(k => {
        ...
        if (this.rowSelection) {
          return props[k] = {
            ...
            onChange: (selectedRowKeys, selectedRows) => {
              this.updateSelect(selectedRowKeys, selectedRows);
              base["default"].isFunction(this[k].onChange) && this[k].onChange(selectedRowKeys, selectedRows);
            }
          };
        }
        return props[k] = null;
    ...
    return h("a-table", helper.helper([{}, {
      props,
      scopedSlots: {
        ...this.$scopedSlots
      },
    ...
    }]), [Object.keys(this.$slots).map(name => h("template", {
      "slot": name
    }, [this.$slots[name]]))]);
  }
};
